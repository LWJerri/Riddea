...
export default class extends CommandInterface {
  ...
  async run(ctx: ContextCallbackWithData) {
    ...
    async function API(callback?: string) {
      if (!callback || callback == "NEW_SHINOBU_WAIFUPICS")
        return await waifuPicsApi({ endPoint: "sfw/shinobu", amount: 10 });
    }
    ...
    await ctx.reply(
      ctx.i18n.translate("bot.main.newPack.images", { pack: ctx.i18n.translate("bot.packs.shinobu") }),
      keyboard,
    );
  }
}
